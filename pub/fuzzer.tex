...
We collected the results of these operations in an 86,627 row table,
...
Comparative language evaluation has a long and colorful history; Kernighan \cite{Ker81} points out Pascal's design and syntax flaws. A comparison of Algol 60, FORTRAN, Pascal and Algol 68 is presented in reference \cite{BOJO80}.

Different behaviour and productivity issues are pointed out in reference \cite{RWSB68}. It states that high-level abstractions of PL/I helped programmers to write fewer program statements, but with higher frequency of errors. To support this, a programmer implemented twice, seven benchmark problems in PL/I and in one other language; Cobol, Fortran and Jovial. The aforementioned publication correlates directly the language abstractions with the user productivity and efficiency, in terms of valid program code.

In reference \cite{PREC00} seven different languages are also compared in terms of reliability, program length among others. This time modern languages are included in the comparison, like C, C++ and Java.
...
Fuzzing techniques are used mainly to detect software security vulnerabilities and 
improve software reliability \cite{TJC08,GODE07}. Several tools and techniques \cite{WWGZ11}
have been developed, introducing concepts like \textit{directed fuzz testing} \cite{GLRI09}.

{\em Grammar-based white box fuzzing} \cite{God08},
takes into account the input language's grammar to fuzz the input in
ways that are syntactically correct. This results in a higher rate of successful fuzzing and the location
of deeper problems.

Another interesting approach is H-fuzzing \cite{ZWZH11}, which is a heuristic method that examines the execution paths
of the program to achieve higher path coverage. 
...
Our approach fuzzes the program with pseudo-random perturbations based in same cases on knowledge of
lexical tokens.

Random Testing \cite{HAM06} seems to be promising solution that can partially deal with the aforementioned problem,
but still it is not widely adopted outside the academic fields \cite{GGBO07}, since the techniques it 
introduces are difficult to be applied in complex systems and achieve good code coverage, at a significant cost \cite{RAWO06}.
...
Our experiment aims to exhibit the fault tolerance of each language and use their features such as 
their type systems to our advantage \cite{LYU95,KOKR07}. 
